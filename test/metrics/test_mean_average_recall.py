...
from supervision.metrics import MetricTarget, MeanAverageRecall
...
PREDICTIONS = [
    ...
    np.array([
        ...
        [332, 162, 478, 428, 0.58, 0],   # TP? (slight shift) treat as TP if IoU high enough; assume OK
...
def mock_detections_list(boxes_list):
    return [Detections(xyxy=boxes[:, :4], confidence=boxes[:, 4], class_id=boxes[:, 5].astype(int)) for boxes in boxes_list]

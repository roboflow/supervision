...
def test_as_coco_annotation_ids(tmp_path):
    ...
    dataset = DetectionDataset(
        ...
        annotations={
            str(image1_path): mock_detections(
                xyxy=[[0, 0, 10, 10], [10, 10, 20, 20]], class_id=[0, 1]
            ),
            str(image2_path): mock_detections(
                xyxy=[[0, 0, 10, 10], [10, 10, 20, 20]], class_id=[1, 0]
            ),
        },
    )
    ...
    assert len(all_ids) == len(set(all_ids)), (
        "Duplicate annotation IDs found across splits"
    )

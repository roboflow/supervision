...
from supervision.detection.base_annotate import BaseAnnotator
...
class CircleAnnotator(BaseAnnotator):
    ...
    def annotate(
        ...
        for i in range(len(detections)):
            ...
            text_y = center[1] - distance - text_size[1] - 2

            cv2.rectangle(
                img=scene,
                pt1=(int(text_x - 2), int(text_y - 2)),
                pt2=(int(text_x + text_size[0] + 2), int(text_y + text_size[1] + 2)),
                color=color.as_bgr(),
                thickness=cv2.FILLED,
            )
            cv2.putText(
                img=scene,
                text=text,
                org=(int(text_x), int(text_y + text_size[1])),
                fontFace=font,
                fontScale=self.text_scale,
                color=self.text_color.as_rgb(),
                thickness=self.text_thickness,

...
def rotate_polygon_2d(
    polygon: np.ndarray, angle: float, rotation_center: Optional[Point] = None
) -> np.ndarray:
    """
...
def rotate_polygon_3d(
    ...
    rotation_center: Optional[np.array] = None,
) -> np.ndarray:
    """
    ...
    Parameters:
        ...
        angles (tuple(float, float, float)): Angles in radiance.
        Angle_x, angle_y and angle_z, that is,
        the first angle specifies the fracturing

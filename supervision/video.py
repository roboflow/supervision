...
import logging
from typing import Any, Callable, Generator, Optional, Protocol, Tuple, Union
import numpy as np
...
class Video:
    ...
    def frames(self, stride: int = 1, start: int = 0, end: Optional[int] = None, resolution_wh: Optional[Tuple[int, int]] = None) -> Generator[np.ndarray, None, None]:
    ...
    def save(self, target_path: str, callback: Optional[Callable[[np.ndarray, int], np.ndarray]] = None, show_progress: bool = False, fps: Optional[float] = None, resolution_wh: Optional[Tuple[int, int]] = None, codec: str = "mp4v") -> None:
        info = self.info
        if fps is not None:
            info = VideoInfo(info.width, info.height, fps, info.total_frames)
        if resolution_wh is not None:
            info = VideoInfo(resolution_wh[0], resolution_wh[1], info.fps, info.total_frames)
    ...
    def sink(self, target_path: str, info: Optional[VideoInfo] = None, codec: str = "mp4v") -> Writer:
        if info is None:
            info = self.info

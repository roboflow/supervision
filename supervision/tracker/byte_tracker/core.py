...
class ByteTrack:
    ...
    def _update_tracklets(self, tensors: np.ndarray,return_track_ids=False) -> Union[List[STrack],np.ndarray]:
        ...
        Parameters:
            tensors (np.ndarray): The new tensors to update with.

            return_track_ids (bool): If True, returns an array of track IDs
                corresponding to each detection. If False (default), returns a list
                of updated STrack objects.

        Returns:
            If return_track_ids is True:
                np.ndarray: Tracker ID for each tensor.
            If return_track_ids is False:
                List[STrack]: Updated tracks.
        ...
        if return_track_ids:
            ...
            tracked_ids = [None] * len(detection_index)

...
def yolo_annotations_to_detections(
    ...
    class_id, relative_xyxy, relative_polygons = [], [], []
    w, h = resolution_wh
    for line in lines:
        values = line.split()
        ...
        if len(values) == 5 and not force_segmentations:
            ...
            box = parse_box(values=values[1:])
            relative_xyxy.append(box)
            relative_polygons.append(box_to_polygon(box=box))
    ...
    polygons = [
        (relative_polygon * np.array(resolution_wh)).astype(int)
        for relative_polygon
        in relative_polygons
    ]

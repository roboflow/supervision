...
Dataset Split Configuration:
    ...
    random_indices = np.random.choice(
        len(image_ids), size=len(image_ids), replace=False
    )
    ...
    # Save split files
    split_files = {"train": train_data, "val": val_data, "test": test_data}
    ...
    for split_name, split_data in split_files.items():
        if split_data["images"]:  # Only save non-empty splits
            ...
            print(
                f"Saved {output_file.name}: {len(split_data['images'])} images, {len(split_data['annotations'])} annotations"
            )
    ...
    for split_name, stats in statistics.items():
        if split_name != "total":
            print(
                f"  {split_name.capitalize()}: {stats['images']} images, {stats['annotations']} annotations"
            )
...
def verify_coco_split(
    ...
    try:
        ...
        train = (
            COCO(str(train_annotations)) if Path(train_annotations).exists() else None
        )
        ...
        if len(all_image_ids) != len(set(all_image_ids)):
            ...
            results["details"]["duplicate_images"] = len(all_image_ids) - len(
                set(all_image_ids)
            )
        ...
        if original_image_count != split_image_count:
            ...
            results["details"]["missing_images"] = (
                original_image_count - split_image_count
            )

        # Check category preservation
        original_categories = {cat["id"] for cat in original.cats.values()}
        for split_name, split_data in valid_splits.items():
            split_categories = {cat["id"] for cat in split_data.cats.values()}
            if original_categories != split_categories:
                results["categories_preserved"] = False
                results["details"][f"{split_name}_missing_categories"] = (
                    original_categories - split_categories
                )

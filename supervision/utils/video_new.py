...
from supervision.utils.video_backend import Backend, VideoInfo, Writer, get_backend
        ...
        try:
            ...
            if end is not None and info.total_frames is not None:
                if end > info.total_frames:
                    warnings.warn(f"Requested end frame {end} exceeds total frames {info.total_frames}")
    ...
    def save(
        ...
        **kwargs,
        ...
        output_info = VideoInfo(
            ...
            total_frames=min(max_frames, source_info.total_frames) if max_frames and source_info.total_frames else max_frames,
            codec=codec,
        )

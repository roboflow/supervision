...
class F1Score(Metric):
    ...
    def _compute(self, predictions_list: List[Detections], targets_list: List[Detections]) -> F1ScoreResult:
        ...
        if stats:
            ...
            f1_scores = self._f1_score_per_class(*concatenated_stats)
        else:
            f1_scores = np.empty((0, len(iou_thresholds)), dtype=np.float32)

        return F1ScoreResult(
            f1_scores=f1_scores,
            iou_thresholds=iou_thresholds,
            # classes=targets.class_id,
    ...
    def _f1_score_per_class(
        ...
        f1_scores = np.zeros((num_classes, matches.shape[1]))
        ...
        for class_idx, class_id in enumerate(unique_classes):
            ...
            for iou_level_idx in range(matches.shape[1]):
                prec = precision[iou_level_idx]
                rec = recall[iou_level_idx]
                f1 = (
                    2 * (prec * rec) / (prec + rec + eps)
                )
                f1_scores[class_idx, iou_level_idx] = f1

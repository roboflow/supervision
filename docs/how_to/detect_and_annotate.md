...
Supervision provides a seamless process for annotating predictions generated by various object detection and segmentation models. This guide shows how to perform inference with [RF-DETR](https://github.com/roboflow/rf-detr), [Inference](https://github.com/roboflow/inference), [Ultralytics](https://github.com/ultralytics/ultralytics), or [Transformers](https://github.com/huggingface/transformers). RF-DETR's `predict` method returns a `Detections` object directly; the other frameworks require a conversion step covered in the next section.
...
To run inference, initialize your chosen model and pass the source image to its predict or infer method. Supervision supports RF-DETR, Roboflow Inference, Ultralytics YOLO, and Hugging Face Transformers -- select the tab matching your framework. RF-DETR already returns a `Detections` instance; the other frameworks return a framework-specific object you will convert to `Detections` in the next step.
...
Each supported framework has a dedicated class method on `sv.Detections` that converts raw model output into a unified Supervision object -- except RF-DETR, whose `predict` method returns a `Detections` object directly. For the others, call `from_inference`, `from_ultralytics`, or `from_transformers` depending on the package you used for inference. This normalization step ensures all downstream annotators and filters work identically regardless of the source model.

=== "RF-DETR"

    RF-DETR's `predict` method returns a `Detections` object directly -- no conversion step needed.

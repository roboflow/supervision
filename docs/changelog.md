...
- A release's own version-pinned docs (e.g. `/0.30.2/...`) no longer show the "older version, use latest" banner on the day it ships. The publish workflow now compares the release tag against the repository's tag history and only suppresses the banner when the tag being deployed is the newest stable release, instead of relying solely on the separate `/latest/` alias build to ever be current.

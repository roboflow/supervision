...
- `sv.DetectionDataset.from_pascal_voc` no longer fails on annotations whose coordinates are written as decimals. The loader parsed every `<bndbox>` and `<polygon>` value with `int()`, so one file carrying `<xmin>48.5</xmin>` — or even `<xmin>48.0</xmin>` — aborted the whole load with `ValueError: invalid literal for int() with base 10: '48.5'`. Exporters that keep sub-pixel boxes write VOC this way; Datumaro, which CVAT uses for its exports, is one of them. Box coordinates are now read as floats and keep their precision, and polygon vertices are rounded to the nearest pixel before rasterising, as the YOLO and LabelMe loaders already do. Integer coordinates load exactly as before, and values that are not finite numbers are still rejected.

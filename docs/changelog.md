...
### Fixed
- Fixed [#2427](https://github.com/roboflow/supervision/issues/2427): size-bucketed results of `sv.Precision`, `sv.Recall`, and `sv.F1Score` no longer count out-of-bucket detections as false positives. Detections outside the requested bucket are now ignored, matching COCO evaluation and `sv.MeanAveragePrecision`, and in-bucket targets are matched before out-of-bucket ones so an ignored target cannot steal a prediction. A pixel-perfect detector now scores 1.0 in every bucket.

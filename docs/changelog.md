...
### 0.29.0 <small>Jun 11, 2026</small>

- Added [#2286](https://github.com/roboflow/supervision/pull/2286): [`sv.KeyPoints`](https://supervision.roboflow.com/latest/keypoint/core/#supervision.key_points.core.KeyPoints) gains three new optional fields: `visible` (boolean `(N, K)` mask indicating keypoint visibility), `keypoint_confidence` (per-keypoint `(N, K)` float32 scores), and `detection_confidence` (per-anchor `(N,)` float32 scores). `VertexAnnotator` and `EdgeAnnotator` now automatically skip invisible keypoints when `visible` is set. Four new annotators visualise keypoint uncertainty: [`sv.VertexEllipseAnnotator`](https://supervision.roboflow.com/latest/keypoint/annotators/#supervision.key_points.annotators.VertexEllipseAnnotator), `sv.VertexEllipseAreaAnnotator`, `sv.VertexEllipseHaloAnnotator`, `sv.VertexEllipseOutlineAnnotator` — each draws covariance ellipses at configurable sigma levels. Per-class skeleton support added to `EdgeAnnotator`; multi-skeleton support and label validation added to `VertexLabelAnnotator`.
...
- Added [#2303](https://github.com/roboflow/supervision/pull/2303): `sv.oriented_box_non_max_suppression` and `sv.oriented_box_non_max_merge` are now exported at the top level, enabling direct use without importing from the internal utility module.

- Added [#2289](https://github.com/roboflow/supervision/pull/2289): [`DetectionDataset.as_yolo`](https://supervision.roboflow.com/latest/datasets/core/#supervision.dataset.core.DetectionDataset.as_yolo) now accepts `is_obb=True` to round-trip oriented bounding box datasets without losing the rotation. Previously the save path had no OBB option and silently wrote 5-token axis-aligned bbox lines for datasets loaded with `from_yolo(..., is_obb=True)`, dropping the four corners stored in `detections.data["xyxyxyxy"]`. Re-loading those saved files with `is_obb=True` then crashed the validator.

- Added [#2269](https://github.com/roboflow/supervision/pull/2269): [`HeatMapAnnotator`](https://supervision.roboflow.com/latest/detection/annotators/#supervision.annotators.core.HeatMapAnnotator) `kernel_size` now accepts `None` to disable Gaussian blur entirely.

- Changed [#2306](https://github.com/roboflow/supervision/pull/2306): [`sv.Detections.area`](https://supervision.roboflow.com/latest/detection/core/#supervision.detection.core.Detections.area) now returns the rotated polygon area (shoelace on the four corners) when `data["xyxyxyxy"]` is set, instead of the axis-aligned bounding box area which overestimates by up to ~2× at 45° rotation. Affects annotator z-ordering inside [`MaskAnnotator`](https://supervision.roboflow.com/latest/detection/annotators/#supervision.annotators.core.MaskAnnotator) and [`HaloAnnotator`](https://supervision.roboflow.com/latest/detection/annotators/#supervision.annotators.core.HaloAnnotator), and any user code that filters or sorts OBB detections by area. The mask path and the non-OBB AABB fallback are unchanged.

- Changed [#2303](https://github.com/roboflow/supervision/pull/2303): [`sv.Detections.with_nms`](https://supervision.roboflow.com/latest/detection/core/#supervision.detection.core.Detections.with_nms) and [`sv.Detections.with_nmm`](https://supervision.roboflow.com/latest/detection/core/#supervision.detection.core.Detections.with_nmm) now use oriented-box IoU when `data["xyxyxyxy"]` coordinates are present, fixing silent suppression of crossed/rotated OBB detections. Callers relying on the old AABB behaviour should remove `data["xyxyxyxy"]` before calling these methods; thresholds calibrated against AABB IoU may need adjustment.

- Changed [#2256](https://github.com/roboflow/supervision/pull/2256): [`sv.InferenceSlicer`](https://supervision.roboflow.com/latest/detection/tools/inference_slicer/#supervision.detection.tools.inference_slicer.InferenceSlicer) now serializes OBB-producing callbacks when `thread_workers > 1` to prevent crashes; `thread_workers` validation added; OBB merge order documented.

- Changed [#2270](https://github.com/roboflow/supervision/pull/2270): [`sv.Detections.from_inference`](https://supervision.roboflow.com/latest/detection/core/#supervision.detection.core.Detections.from_inference), `from_ultralytics`, and `from_vlm` now always populate `data["class_name"]` as a string-dtype NumPy array even on empty paths. Previously empty paths returned a missing key or float64 array; downstream code reading `data["class_name"]` on empty detections is now consistent.

- Changed [#2296](https://github.com/roboflow/supervision/pull/2296): YOLO class name loading now sorts `class_names` keyed by stringified integers in numeric order (`"10"` after `"9"`, not after `"1"`). Mixed numeric/non-numeric key sets raise `ValueError`.

- Changed [#2297](https://github.com/roboflow/supervision/pull/2297): [`letterbox_image`](https://supervision.roboflow.com/latest/utils/image/) now supports grayscale `(H, W)` and BGRA `(H, W, 4)` inputs; input array is no longer mutated.

- Changed [#2252](https://github.com/roboflow/supervision/pull/2252): [`sv.process_video`](https://supervision.roboflow.com/latest/utils/video/#supervision.utils.video.process_video) now preserves the audio stream via ffmpeg when available; warns and continues when ffmpeg is missing or fails.

- Deprecated [#2286](https://github.com/roboflow/supervision/pull/2286): `KeyPoints.confidence` is deprecated in favour of `KeyPoints.keypoint_confidence`. Existing code continues to work with a `SupervisionWarning`; removal planned for `supervision-0.32.0`.

- Deprecated [#2294](https://github.com/roboflow/supervision/pull/2294): All public `validate_*` helper functions in `supervision.validators` have been renamed to `_validate_*` (private). The old public names remain as `@deprecated` forwarders through `0.32.0`. `validate_fields_both_defined_or_none` in `supervision.detection.core` is deprecated with removal in `0.31.0`.

- Fixed [#2282](https://github.com/roboflow/supervision/pull/2282): [`oriented_box_iou_batch`](https://supervision.roboflow.com/latest/detection/utils/iou_and_nms/) now allocates the rasterization canvas with correct dimensions (previously x-extents were mapped to canvas height and y-extents to width, causing wrong IoU on non-square canvases). Also adds memory cap (`_MAX_IOU_CANVAS_DIM = 1024`), `overlap_metric` argument, and empty-input guard.

- Fixed [#2276](https://github.com/roboflow/supervision/pull/2276): [`DetectionDataset.as_coco`](https://supervision.roboflow.com/latest/datasets/core/#supervision.dataset.core.DetectionDataset.as_coco) now emits 1-indexed `category_id` and `categories[].id` values as required by the COCO spec, fixing CVAT import failures reported in [#1181](https://github.com/roboflow/supervision/issues/1181).

- Fixed [#2302](https://github.com/roboflow/supervision/pull/2302): YOLO OBB annotation export now validates the `(N, 4, 2)` corner shape and `is_obb=False` passthrough is handled correctly.

- Fixed [#2269](https://github.com/roboflow/supervision/pull/2269): [`HeatMapAnnotator`](https://supervision.roboflow.com/latest/detection/annotators/#supervision.annotators.core.HeatMapAnnotator) raised `RuntimeWarning: invalid value encountered in divide` on empty detections (e.g. first frames before any output); now returns scene unchanged when no activations exist.

- Fixed [#2239](https://github.com/roboflow/supervision/pull/2239): [`sv.Detections.from_paligemma`](https://supervision.roboflow.com/latest/detection/core/#supervision.detection.core.Detections.from_paligemma) and `from_google_gemini_2_0` returned `class_id=None` on empty parse paths (regex no-match, `JSONDecodeError`, empty bounding-box list); now return a zero-length `int` ndarray to match other empty paths.

- Fixed [#2270](https://github.com/roboflow/supervision/pull/2270): [`sv.Detections.from_inference`](https://supervision.roboflow.com/latest/detection/core/#supervision.detection.core.Detections.from_inference) did not set `data["class_name"]` on the empty path, producing a missing key or `float64` array; now always sets a string-dtype empty array.

- Fixed [#2286](https://github.com/roboflow/supervision/pull/2286): `KeyPoints.__getitem__` crashed on NumPy scalar, 0-D ndarray, and bool-mask inputs; `detection_confidence` indexing now uses a normalised row index helper.

- Fixed [#2298](https://github.com/roboflow/supervision/pull/2298): File extension filters in `supervision.utils.file` mishandled dotted extensions, empty extensions, and multi-part suffixes (e.g. `.tar.gz`).

- Fixed [#2297](https://github.com/roboflow/supervision/pull/2297): `letterbox_image` corrupted the alpha channel for RGBA inputs and failed on grayscale `(H, W)` inputs.

- Fixed [#2252](https://github.com/roboflow/supervision/pull/2252): `process_video` lost the audio stream when ffmpeg failed or was missing; `OSError` on temp-file allocation now caught and warned.

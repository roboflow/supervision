...
- `sv.IconAnnotator` and `sv.draw_image` now draw CMYK JPEG and TIFF images in their colors when OpenCV is not installed. Both read images with `cv2.IMREAD_UNCHANGED`, for which OpenCV converts CMYK to color, returning BGR for a JPEG and BGRA with an opaque alpha channel for a TIFF. The OpenCV-free fallback instead returned the four ink channels, cyan, magenta, yellow and black, as if they were blue, green, red and alpha, so an icon was drawn in the wrong colors and only where it had black ink: a CMYK image with no black ink was not drawn at all. The fallback now returns the same arrays as `cv2.imread` for CMYK images. Other images, and every read with OpenCV installed, are unchanged. [#2602](https://github.com/roboflow/supervision/pull/2602)

- `sv.draw_image` now draws a 16-bit PNG in its own colors. It reads an image path with `cv2.IMREAD_UNCHANGED` to keep the alpha channel, and with OpenCV installed that also keeps a 16-bit PNG at 16 bits, so the blend into the 8-bit scene clipped channel values above `255`: a dark gray logo was drawn white, and a red of `200` drew as `255`. A 16-bit image read from a path is now scaled down to 8 bits as it is loaded, as `sv.IconAnnotator` already does for its icons. Eight-bit images, and images passed as arrays, are drawn as before. [#2603](https://github.com/roboflow/supervision/pull/2603)

- `sv.metrics.MeanAverageRecall` now scores mAR@K from each image's K most confident predictions alone. It matched every prediction to the targets first and only then kept the top K, but the matcher pairs by highest IoU rather than by confidence, so a prediction ranked below K could take a target from one ranked within it and leave that one unmatched. Adding a low-confidence duplicate that fitted a target more tightly therefore lowered mAR@1: one target with a top prediction at IoU `0.71` scored mAR@1 `0.5` on its own but `0.0` once a second prediction with confidence `0.1` and IoU `1.0` was added. Each detection limit now matches only its own top K predictions, so mAR@1 and mAR@10 equal the recall of the top 1 and top 10 predictions per image, as documented. mAR@100 changes only for images with more than 100 predictions. [#2604](https://github.com/roboflow/supervision/pull/2604)

- `sv.KeyPoints.as_detections` now leaves key points marked not visible out of the box it derives for each skeleton, as `sv.KeyPoints.with_nms` already does. `visible` is how callers hide unreliable key points without removing them, for example `key_points.visible = key_points.keypoint_confidence > 0.3`, and the key point annotators skip those points, but `as_detections` still stretched each box over every key point that was not `[0, 0]` or non-finite. A pose whose low-confidence joints were predicted outside the person, such as legs below the bottom of the frame, therefore got a box reaching out to them, a different box from the one `with_nms` suppresses with. A skeleton with no visible key point is now dropped, as one with only missing key points already was. Key points without `visible` convert as before. [#2605](https://github.com/roboflow/supervision/pull/2605)
...
- `sv.LineZone.trigger` no longer counts a spurious crossing in the opposite direction when a tracker flickers to the far side of the line for fewer than `minimum_crossing_threshold` frames. A crossing was confirmed whenever the oldest entry of a `minimum_crossing_threshold + 1` frame history differed from every later entry, which never verified that the tracker had been settled on the side it supposedly came from — with `minimum_crossing_threshold=2` the side sequence `A,A,A,B,A,A,A` counted one crossing *into* `A`, the side the object never left, so counts drifted by one per flicker and in the wrong direction, and three such flickers gave `in_count=3` instead of `0`. Crossings are now measured against the last side a tracker was confirmed on, and once a tracker has a confirmed side, a new one replaces it only after it has been held for the full `minimum_crossing_threshold`, so sub-threshold excursions mid-track are discarded. The first side a tracker is observed on — at the start of its life, and again after it has been absent long enough to be forgotten — seeds its reference without that confirmation, so a single noisy frame there still decides the side later crossings are measured from. Sustained crossings, `minimum_crossing_threshold=1` (the default), and per-tracker isolation are unchanged. [#2600](https://github.com/roboflow/supervision/pull/2600)

...
- Changed [#2312](https://github.com/roboflow/supervision/pull/2312): [`sv.Detections.with_nmm`](https://supervision.roboflow.com/latest/detection/core/#supervision.detection.core.Detections.with_nmm) now computes the merged oriented bounding box as the minimum-area rotated rectangle (`cv2.minAreaRect`) enclosing all corners from every detection in a merge group, rather than keeping the highest-confidence detection's original OBB geometry. The axis-aligned `xyxy` field is updated accordingly. For zero-rotation OBBs this degenerates to the axis-aligned union (consistent with AABB NMM); for rotated OBBs the merged geometry is the MARC. Detections without `data["xyxyxyxy"]` and single-detection groups are unaffected.

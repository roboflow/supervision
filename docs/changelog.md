...
- `sv.LineZone.trigger` no longer inflates crossing counts for unconfirmed tracks. It keyed crossing state by `tracker_id`, and treated every detection with `tracker_id = -1` as the same track. Trackers that report `-1` for tracks they have not yet confirmed (e.g. `roboflow/trackers`' `ByteTrackTracker`) can have several distinct unconfirmed objects on screen at once, so their independent motion collapsed into one phantom track that crossed the line repeatedly, silently inflating `in_count`/`out_count`. Detections with `tracker_id < 0` are now skipped before they touch crossing history, so they no longer affect counts or interfere with confirmed tracks sharing the same frame. Confirmed tracks (`tracker_id >= 0`) are counted exactly as before. [#2579](https://github.com/roboflow/supervision/pull/2579)

...
- `sv.draw_image` now draws a 16-bit PNG in its own colors. It reads an image path with `cv2.IMREAD_UNCHANGED` to keep the alpha channel, and with OpenCV installed that also keeps a 16-bit PNG at 16 bits, so the blend into the 8-bit scene saturated every nonzero channel to `255`: a dark gray logo was drawn white, and a red of `200` drew as `255`. A 16-bit image read from a path is now scaled down to 8 bits as it is loaded, as `sv.IconAnnotator` already does for its icons. Eight-bit images, and images passed as arrays, are drawn as before. [#2603](https://github.com/roboflow/supervision/pull/2603)

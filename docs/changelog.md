...
- `sv.LineZone.trigger` no longer counts detections whose `tracker_id` is negative (such as the `-1` that ByteTrack-style trackers assign to tracks that have not been confirmed yet). Every `-1` was previously collapsed into a single shared track, so distinct unconfirmed objects on a busy frame silently inflated the in/out counts; such detections are now skipped before any crossing state is updated. ([#2578](https://github.com/roboflow/supervision/issues/2578))

...
- `sv.plot_images_grid` now plots a single image in a `grid_size=(1, 1)` grid. It iterated over the grid's axes with `axes.flat`, but Matplotlib's `plt.subplots` returns a lone `Axes` rather than an array when the grid has one row and one column, so the call failed with `AttributeError: 'Axes' object has no attribute 'flat'` before drawing anything. Code that sizes the grid from the number of images, such as `grid_size=(1, len(images))`, hit this whenever there was only one image. The grid's axes are now always requested as an array. Grids with more than one cell plot as before.

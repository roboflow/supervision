...
class LineZone:
    ...
    def _compute_anchor_sides(
        ...
        if detections.tracker_id is None:
            raise ValueError("Detections must have a tracker_id.")
    ...
    def _update_class_id_to_name(self, detections: Detections) -> None:
        ...
        if class_names is not None and detections.class_id is None:
            raise ValueError("class_names are provided but class_id is None.")
...
class LineZoneAnnotator:
    ...
    def _draw_oriented_label(
        ...
        if label_image.shape[0] != label_image.shape[1]:
            raise ValueError("label_image must be a square image.")
